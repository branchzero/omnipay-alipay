...
class BatchTransCompletePurchaseRequest extends AbstractRequest {
    ...
    public function getData()
    {
        ...
        $this->validateRequestParams('pay_user_name', 'pay_user_id', 'batch_no','pay_account_no');
    ...
    protected function getParamsSignature($data, $sign_type = false)
    {
        ksort($data);
        reset($data);
        $query_string = http_build_query($data);
        $query_string = urldecode($query_string);
        $sign_type = $sign_type ? : $this->getSignType();
        if ($sign_type == 'MD5')
        {
    ...
    public function sendData($data)
    {
        ...
        return $this->response = new BatchTransCompletePurchaseResponse($this, $data);
    }
    ...
    {
        if ($this->getRequestParam('sign') == $this->getParamsSignature($this->getParamsToSign(), $this->getRequestParam('sign_type')))
        {
            return true;
        }
        else
